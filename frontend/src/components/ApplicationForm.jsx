...
const ApplicationForm = () => {
  ...
  const handleSubmit = async () => {
    if (validateSection()) {
      ...
      try {
        ...
        // Basic token format check (JWT should be three base64 parts separated by dots)
        if (!token.match(/^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/)) {
          showPopup("Invalid token format. Please log in again.", "error");
          localStorage.removeItem("token"); // Clear invalid token
          return;
        }
        console.log("Token (redacted):", token.slice(0, 10) + "..."); // Log token for debugging
        ...
        // Ensure documents array contains valid entries
        ...
        // Append files under the "documents" field
  ...
  const handleSectionClick = (index) => {
    if (index <= activeIndex || completedSections[index - 1]) {
  ...
  const handleInputChange = (e, level, index) => {
    ...
    if (name === "year") {
      sanitizedValue = value.trim(); // Remove leading/trailing spaces
      if (sanitizedValue && !/^\d{0,4}$/.test(sanitizedValue)) {
        return; // Allow only up to 4 digits
      }
    }
        ...
        return (
          <>
            ...
            <select {...fieldProps}>
              ...
              {/* Add options dynamically if provided in custom field configuration */}
            </select>
